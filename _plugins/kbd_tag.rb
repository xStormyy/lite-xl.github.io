...
module Jekyll
  ...
  class KBDTag < Liquid::Tag
    ...
    def initialize(tag_name, markup, tokens)
      if markup.strip.start_with?("!")
        @target = markup.gsub(/^\s*\!|\s*$/, "")
      else
        @combos = create_combos(markup)
      end
      super
    end

    def create_combos(markup)
      markup.split(%r{ / }).inject([]) do |combos, combo|
        mods = []
        key = ''
        combo.clean_combo!
