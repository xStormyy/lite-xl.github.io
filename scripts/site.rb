...
Dir.glob("locales/*").map { |locale| File.write(File.basename(locale) + ".html", File.read("#{locale}/template.html").gsub("{{ pages }}", "<style type='text/css'>" + Rouge::Themes::Base16.mode(:dark).render(scope: 'pre') + "</style>" +
